...
const Appointment = ({
  ...
  const {
    styles,
    ACCESS_TOKEN
  } = options; // Saves all the appointments

  const [appointmentList, setAppointmentList] = useState([]);
  const [isLoading, setIsLoading] = useState(false);
  const [appointmentDetails, setAppointmentDetails] = useState("");
  const [isModalVisible, setModalVisible] = useState(false); // Date selected from calendar

  const [filterDate, setFilterDate] = useState(""); // Filtered appointments according to their dates

  const [filteredAppointments, setFilteredAppointments] = useState([]); // Fetches all the created appointments from backend
  ...
  useEffect(() => {
    if (filterDate) {
      const appointmentFilter = appointmentList.filter(item => item.selected_date === filterDate);
  ...
  return <View style={styles.appointmentListContainer}>
      {isLoading && <Loader />}
      <View style={styles.calendarTextContainer}>
        <Text style={styles.calendarText}>Calendar</Text>
      </View>
      <CalendarStrip daySelectionAnimation={{
      type: "background",
      duration: 10,
      highlightColor: "#E6E6E6"
    }} style={styles.calendarStrip} onDateSelected={date => setFilterDate(formatDate(new Date(date)))} />
      <View>
      ...
      <AppointmentModal setModalVisible={setModalVisible} modalItem={appointmentDetails} modalVisible={isModalVisible} />
    </View>;
};
