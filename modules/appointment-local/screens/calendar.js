...
const Calendar = ({
  ...
  const today = useRef(new Date()); // Open state for duration dropdown

  const [open, setOpen] = useState(false);
  const [duration, setDuration] = useState("00:30:00"); // Marked dates on calendar
  ...
  return <SafeAreaView>
      <ScrollView>
        <View style={styles.ph10}>
          <CalendarList minDate={today.current.toDateString()} horizontal={true} pagingEnabled={true} calendarWidth={370} onDayPress={daySelector} markedDates={markedDates.markedDates} />
