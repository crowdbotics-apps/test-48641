import moment from "moment"; // This function formats the date passed into a specific manner
...
export const checkLoading = (...params) => params.some(param => param.loading === "pending"); // This function gets the ending time of the appointment by getting its start time and total duration in params
...
export const getEndTime = (startTime, duration) => {
  if (startTime && duration) {
    ...
    const duration2 = moment.duration(duration); // Add the durations together

    const totalDuration = duration1.add(duration2); // Get the total time in "HH:mm:ss" format

    const totalTime = moment.utc(totalDuration.asMilliseconds()).format("HH:mm:ss");
    return totalTime;
  }
}; // This function calculates the duration between start and end time passed through params

export const getDuration = (startTime, endTime) => moment.utc(moment(endTime, "HH:mm:ss").diff(moment(startTime, "HH:mm:ss"))).format("HH:mm:ss");
