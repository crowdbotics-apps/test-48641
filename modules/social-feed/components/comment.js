...
export const Comment = ({
  ...
  const [comment, setComment] = useState(""); // Save the comment reference for reply
  ...
  return <View style={styles.commentMainContainer}>
      <Text>Comments</Text>
      {comments?.map(item => {
      return <View style={styles.commentContainer} key={item.key}>
            <View style={styles.singleCommentHeader}>
              <TouchableOpacity style={styles.commentUserContainer} onPress={() => {
            navigation.navigate("SocialProfileScreen", {
              id: item?.user?.id
                ...
                uri: item.user.photo
              } : require("../assets/user.png")} style={styles.commentUserImage} />
                </View>
                <Text style={styles.commentUserText}>{item?.user?.name}</Text>
              </TouchableOpacity>

              <View style={_styles.lxlYUgyH}>
                <TouchableOpacity>
                  <Image source={require("../assets/dots.png")} style={_styles.KwxUAplB} />
                </TouchableOpacity>
              </View>
            </View>
            ...
            <View style={styles.postcontainer}>
              ...
              item.liked ? unLikePostComment(item.id) : likePostComment(item.id);
            }} style={styles.commentLikeButton}>
                  <Image source={item.liked ? require("../assets/unlike.png") : require("../assets/like.png")} style={styles.imageIcons} />
                  <Text style={styles.mh10}>{item.likes}</Text>
                </TouchableOpacity>
                <TouchableOpacity style={styles.commentReplyButton} onPress={() => {
            ...
            return <View style={styles.commentReplyContainer} key={item.key}>
                    <View style={styles.commentReplyMain}>
                      <TouchableOpacity style={styles.commentReplyUserContainer} onPress={() => {
                  navigation.navigate("SocialProfileScreen", {
                    id: item?.user?.id
                  });
                }}>
                        <View style={styles.commentReplyUserImageContainer}>
                          <Image source={item?.user?.photo ? {
                      uri: item.user.photo
                    } : require("../assets/user.png")} style={styles.commentReplyUserImage} />
                        </View>
                        <Text style={styles.commentReplyUserText}>
                          {item?.user?.name}
                        </Text>
                      </TouchableOpacity>
                      <View style={_styles.rbrQwbqJ}>
                        <TouchableOpacity>
                          <Image source={require("../assets/dots.png")} style={_styles.aZSYawck} />
                        </TouchableOpacity>
                      </View>
                    </View>
                    ...
                    item.liked ? unLikePostComment(item.id) : likePostComment(item.id);
                        ...
                        <TouchableOpacity style={_styles.RcsKHoXE} onPress={() => {
                    refComment.current = item?.id;
                    setComment("@" + item.user.name);
                  }}>
                          <Image source={require("../assets/group.png")} style={[styles.imageIcons, styles.mr10]} />
      ...
      <View style={styles.commentTextInput}>
        <TextInput style={[styles.replyInput, _styles.DKxenYRL]} placeholder="Add a comment" onChangeText={newText => setComment(newText)} defaultValue={comment} />
        <TouchableOpacity onPress={() => {
        submitComment();
      }} style={[styles.commentSubmitButton, !comment && {
        backgroundColor: "lightgray"
      }]}>
          <Image source={require("../assets/send.png")} style={styles.userImage} />
        </TouchableOpacity>
      </View>
    </View>;
};
