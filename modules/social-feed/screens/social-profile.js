...
const SocialProfileScreen = ({
  ...
  const {
    styles
  } = useContext(OptionsContext); // Get user profile data from the store.

  const {
    entities
  } = useSelector(state => state.Social.getUserProfile);
  ...
  return <ScrollView>
      <View style={styles.socialProfileContainer}>
        ...
        <View style={styles.followingSection}>
          ...
          {entities?.is_owner && <>
              <TouchableOpacity style={styles.textarea} onPress={() => {
            openFollowers();
          }}>
                <Image style={styles.postIcon} source={require("../assets/user.png")} />
            ...
            openFollowing();
                ...
                <Image style={styles.followingIcon} source={require("../assets/user.png")} />
                <Text style={styles.followingText}>{following} Following</Text>
              </TouchableOpacity>
            </>}
          {!entities?.is_owner && <>
              {entities?.i_follow ? <TouchableOpacity style={styles.textarea} onPress={unFollowCurrentUser}>
                  <Image style={styles.postIcon} source={require("../assets/user.png")} />
                  <Text>UnFollow</Text>
                </TouchableOpacity> : <TouchableOpacity style={styles.textarea} onPress={followCurrentUser}>
                  <Image style={styles.followingIcon} source={require("../assets/user.png")} />
        ...
        <View style={styles.pt30}>
          <FlatList data={posts && posts} style={styles.galleryRow} numColumns={3} renderItem={({
          item
        }) => <View style={styles.postItemView}>
                <Post onPress={() => {
            pressed(item.id);
          }} data={item} />
              </View>} keyExtractor={item => item.id} onRefresh={getPostUser} refreshing={callbackVariable} />
        </View>
      </View>
    </ScrollView>;
};
...
const Post = props => {
  ...
  return <TouchableHighlight onPress={props.onPress} style={styles.galleryPost} underlayColor="#DDDDDD">
      <Image style={styles.editIcon} source={props?.data?.media?.[0]?.image ? {
...
const ProfileImage = props => {
  ...
  return <TouchableHighlight onPress={props.onPress} underlayColor="#DDDDDD">
      <View style={styles.profileImageContainer}>
        <Image style={styles.userImage} resizeMode="contain" source={require("../assets/user.png")} />
      </View>
    </TouchableHighlight>;
};